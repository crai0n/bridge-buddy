...
impl MoveGenerator {
    ...
    fn calc_priority_leading_nt<const N: usize>(moves: &mut [DdsMove], state: &VirtualState<N>) {
        let suit_weights = Self::suit_weights_for_leading_nt(state);
    ...
    fn calc_priority_leading_trump<const N: usize>(moves: &mut [DdsMove], state: &VirtualState<N>, trump_suit: Suit) {
        let suit_weights = Self::suit_weights_for_leading_trump(state, trump_suit);
        // let player = state.next_to_play();
    ...
    fn suit_weights_for_leading_nt<const N: usize>(state: &VirtualState<N>) -> [isize; 4] {
        let player = state.next_to_play();
        let lho = player + 1;
    ...
    fn suit_weights_for_leading_trump<const N: usize>(state: &VirtualState<N>, trump_suit: Suit) -> [isize; 4] {
        let player = state.next_to_play();
        let lho = player + 1;
        let partner = player + 2;
        let rho = player + 3;
