    ...
    fn suit_weights_for_leading<const N: usize>(state: &VirtualState<N>) -> [isize; 4] {
        ...
        SUIT_ARRAY.map(|suit| {
            if my_hand.is_void_in(suit) {
                ...
                let partner_owns_2nd_and_we_own_3rd = state.owner_of_runner_up_in(suit) == Some(partner)
                    && state.owner_of_runner_up_in(suit) == Some(player);

                let we_own_2nd_and_partner_owns_3rd = state.owner_of_runner_up_in(suit) == Some(player)
                                ...
                                && match (lho_can_ruff, rho_can_ruff) {
                                    (true, true) => {
                                        partners_hand.highest_card_in(trump_suit)
                                            > lhos_hand.highest_card_in(trump_suit)
                                            && partners_hand.highest_card_in(trump_suit)
                ...
                let opp_counts = [lho, rho].map(|opponent| {
                ...
                bonus - ((opp_counts[0] + opp_counts[1]) * 16) / 15
            }
        })
    }
}
