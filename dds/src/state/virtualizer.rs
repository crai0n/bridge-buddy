...
impl Virtualizer {
    ...
    #[allow(dead_code)]
    pub fn virtual_to_absolute2(&self, virtual_card: VirtualCard) -> Option<Card> {
        let suit = virtual_card.suit;
    ...
    #[allow(dead_code)]
    pub fn absolute_to_virtual2(&self, card: Card) -> Option<VirtualCard> {
        let suit = card.suit;
        let out_of_play = self.played[suit as usize];
...
mod test {
    ...
    #[test_case(Rank::Two, 0b0000_0011_0100_1000, Some(VirtualRank::Six))]
    #[test_case(Rank::Two, 0b0000_0011_0100_1001, None)]
    fn virtual_given_played2(rank: Rank, played: u16, expected: Option<VirtualRank>) {
        let played = SuitField::from_u16(played);
        let array = [played; 4];
        let virtualizer = Virtualizer::new(array);
        let suit = Suit::Clubs;
        let expected = expected.map(|rank| VirtualCard { rank, suit });
        let relative = virtualizer.absolute_to_virtual2(Card { suit, rank });
        assert_eq!(relative, expected)
    }
    ...
    #[test_case(VirtualRank::Six, 0b0000_0011_0100_1000, Some(Rank::Two))]
    #[test_case(VirtualRank::Jack, 0b0000_0011_0100_1001, Some(Rank::Nine))]
    fn absolute_given_played2(rank: VirtualRank, played: u16, expected: Option<Rank>) {
        let played = SuitField::from_u16(played);
        let array = [played; 4];
        let virtualizer = Virtualizer::new(array);
        let suit = Suit::Clubs;
        let expected = expected.map(|rank| Card { rank, suit });
        let absolute = virtualizer.virtual_to_absolute2(VirtualCard { suit, rank });
        assert_eq!(absolute, expected)
    }
}
